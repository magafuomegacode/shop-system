...
@section('content')
    ...
    <div class="bg-gradient-to-br from-stone-800 via-stone-700 to-stone-800 rounded-2xl p-5 sm:p-6 shadow-2xl border border-stone-600/50">
        <h2 class="text-white text-xl font-bold mb-1">Edit User</h2>
        <p class="text-stone-300 text-sm mb-6">{{ $user->full_name }} · {{ ucfirst($user->role) }}</p>
        ...
        <form method="POST" action="{{ route('users.update', $user) }}" class="space-y-5">
            ...
            <div>
                <label class="block text-xs font-semibold text-stone-300 mb-2">Full Name</label>
            ...
            <div>
                <label class="block text-xs font-semibold text-stone-300 mb-2">Username</label>
                <input type="text" name="username" value="{{ old('username', $user->username) }}" required
                       class="w-full bg-stone-800/60 backdrop-blur border border-stone-600/50 text-white placeholder-stone-400/50 px-4 py-3 rounded-xl outline-none focus:border-lime-400 focus:ring-2 focus:ring-lime-400/30 transition">
            </div>

            <div>
                <label class="block text-xs font-semibold text-stone-300 mb-2">Email</label>
                <input type="email" name="email" value="{{ old('email', $user->email) }}"
                       class="w-full bg-stone-800/60 backdrop-blur border border-stone-600/50 text-white placeholder-stone-400/50 px-4 py-3 rounded-xl outline-none focus:border-lime-400 focus:ring-2 focus:ring-lime-400/30 transition">
            </div>

            <div>
                <label class="block text-xs font-semibold text-stone-300 mb-2">Phone</label>
            ...
            <div class="flex items-center gap-2">
                ...
                <label for="is_active" class="text-sm text-stone-200">Active account</label>
            </div>
