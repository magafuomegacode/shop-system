...
@section('content')
        ...
        <div class="relative z-10 flex items-center gap-4">
            ...
            <div class="min-w-0">
                <div class="flex items-center gap-2 flex-wrap">
                    ...
                    @if($user->isAdmin())
                        <span class="text-[10px] px-2 py-0.5 rounded-full bg-purple-500/30 text-purple-200 border border-purple-400/40 font-semibold uppercase">Admin</span>
                    @elseif($user->isOwner())
                        <span class="text-[10px] px-2 py-0.5 rounded-full bg-blue-500/30 text-blue-200 border border-blue-400/40 font-semibold uppercase">Owner</span>
                    @else
                        <span class="text-[10px] px-2 py-0.5 rounded-full bg-lime-500/20 text-lime-200 border border-lime-500/40 font-semibold uppercase">Cashier</span>
                    @endif

                    @if(!$user->is_active)
                        <span class="text-[10px] px-2 py-0.5 rounded-full bg-red-500/30 text-red-200 border border-red-400/40 font-semibold uppercase">Blocked</span>
                    @endif
                </div>
                <p class="text-stone-300 text-sm mt-1 truncate">
    ...
    {{-- Success message --}}
    @if(session('success'))
        <div class="fade-in-up d-2 bg-green-500/20 border border-green-500/40 text-green-200 px-4 py-3 rounded-xl mb-4 text-sm flex items-center gap-2">
    ...
    <div class="fade-in-up d-2 grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
        ...
        <div class="bg-gradient-to-br from-stone-800 via-stone-700 to-stone-800 rounded-2xl p-4 shadow-2xl border border-stone-600/50">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-xl bg-lime-500/20 border border-lime-500/40 flex items-center justify-center flex-shrink-0">
                    <svg class="w-5 h-5 text-lime-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M16 12a4 4 0 10-8 0 4 4 0 008 0zm0 0v1.5a2.5 2.5 0 005 0V12a9 9 0 10-9 9m4.5-1.206a8.959 8.959 0 01-4.5 1.207"/>
                    </svg>
                </div>
                <div class="min-w-0">
                    <p class="text-stone-300 text-xs font-medium">Email</p>
                    <p class="text-white text-sm mt-0.5 truncate font-medium">{{ $user->email ?? '—' }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-stone-800 via-stone-700 to-stone-800 rounded-2xl p-4 shadow-2xl border border-stone-600/50">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-xl bg-green-500/20 border border-green-500/40 flex items-center justify-center flex-shrink-0">
                    <svg class="w-5 h-5 text-green-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
                    </svg>
                </div>
                <div class="min-w-0">
                    <p class="text-stone-300 text-xs font-medium">Phone</p>
                    <p class="text-white text-sm mt-0.5 truncate font-medium">{{ $user->phone ?? '—' }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-stone-800 via-stone-700 to-stone-800 rounded-2xl p-4 shadow-2xl border border-stone-600/50">
            <div class="flex items-center gap-3">
                ...
                <div class="min-w-0">
                    <p class="text-stone-300 text-xs font-medium">
                        @if($user->isCashier())
                            Store Access
                        @else
                            Store
                    ...
                    <p class="text-white text-sm mt-0.5 truncate font-medium">
                        @if($user->isCashier())
                            All Stores
                        @elseif($user->store)
                            {{ $user->store->name }}
        ...
        <div class="bg-gradient-to-br from-stone-800 via-stone-700 to-stone-800 rounded-2xl p-4 shadow-2xl border border-stone-600/50">
            <div class="flex items-center gap-3">
                ...
                <div class="min-w-0">
                    <p class="text-stone-300 text-xs font-medium">Status</p>
                    <p class="text-sm mt-0.5 {{ $user->is_active ? 'text-green-400' : 'text-red-400' }} font-semibold">
    ...
    <div class="fade-in-up d-3 bg-gradient-to-br from-stone-800 via-stone-700 to-stone-800 rounded-2xl p-5 mb-4 shadow-2xl border border-stone-600/50">
        ...
        <div class="space-y-3">
            ...
            <div class="flex items-center justify-between py-2 border-b border-stone-600/50">
                <span class="text-stone-300 text-sm">Created at</span>
                <span class="text-white text-sm font-medium">{{ $user->created_at->format('d M Y, H:i') }}</span>
            </div>

            <div class="flex items-center justify-between py-2 border-b border-stone-600/50">
                <span class="text-stone-300 text-sm">Last login</span>
            ...
            @if($user->creator)
                <div class="flex items-center justify-between py-2 border-b border-stone-600/50">
                    <span class="text-stone-300 text-sm">Created by</span>
                    <span class="text-white text-sm font-medium">{{ $user->creator->full_name }}</span>
                </div>
            @endif
    ...
    <div class="fade-in-up d-4 flex flex-wrap gap-3">
        ...
        @if($user->id !== auth()->id())
            <form method="POST" action="{{ route('users.toggle-active', $user) }}">
                @csrf
                @method('PATCH')
                <button class="{{ $user->is_active
                    ? 'bg-yellow-500/20 hover:bg-yellow-500/30 border border-yellow-500/40 text-yellow-200'
                    : 'bg-green-500/20 hover:bg-green-500/30 border border-green-500/40 text-green-200' }}
                    px-4 py-2.5 rounded-xl text-sm font-semibold transition flex items-center gap-2">
                    @if($user->is_active)
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"/>
                        </svg>
