...
@section('content')
    ...
    <div class="fade-in-up d-1 bg-gradient-to-br from-stone-800 via-stone-700 to-stone-800 rounded-2xl p-5 mb-6 relative overflow-hidden shadow-2xl border border-stone-600/50">
        ...
        <div class="relative z-10 flex items-center gap-4">
            <div class="w-16 h-16 rounded-2xl bg-gradient-to-br from-yellow-600 to-lime-600 flex items-center justify-center text-white font-bold text-2xl flex-shrink-0 shadow-lg">
                {{ strtoupper(substr($user->full_name, 0, 1)) }}
            </div>
            <div class="min-w-0">
                <h2 class="text-white text-xl sm:text-2xl font-bold truncate">{{ $user->full_name }}</h2>
                <p class="text-stone-300 text-xs sm:text-sm mt-0.5 capitalize">
    ...
    <form method="POST" action="{{ route('profile.update') }}"
          class="fade-in-up d-2 bg-gradient-to-br from-stone-800 via-stone-700 to-stone-800 rounded-2xl p-5 sm:p-6 space-y-5 shadow-2xl border border-stone-600/50 mb-4">
        @csrf
        @method('PUT')
        ...
        <div>
            <label for="full_name" class="block text-xs font-semibold text-stone-300 mb-2">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
                <label for="email" class="block text-xs font-semibold text-stone-300 mb-2">Email</label>
            ...
            <div>
                <label for="phone" class="block text-xs font-semibold text-stone-300 mb-2">Phone</label>
        ...
        <div>
            <label class="block text-xs font-semibold text-stone-300 mb-2">Username (read-only)</label>
            <div class="flex items-center gap-2 bg-stone-800/40 border border-stone-600/30 rounded-xl px-4 py-3">
                <span class="text-stone-400 text-sm">@</span>
                <span class="text-white font-mono font-semibold">{{ $user->username }}</span>
            </div>
            <p class="text-stone-400/70 text-xs mt-1">Username cannot be changed</p>
        </div>

        <div class="flex items-center gap-3 pt-3 border-t border-stone-600/50">
            <button type="submit"
    ...
    <form method="POST" action="{{ route('profile.change-password') }}"
          class="fade-in-up d-3 bg-gradient-to-br from-stone-800 via-stone-700 to-stone-800 rounded-2xl p-5 sm:p-6 space-y-5 shadow-2xl border border-stone-600/50">
        @csrf
        @method('PUT')
        ...
        <div>
            <label for="current_password" class="block text-xs font-semibold text-stone-300 mb-2">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
                <label for="new_password" class="block text-xs font-semibold text-stone-300 mb-2">
                    New Password <span class="text-red-400">*</span>
                </label>
                <input type="password" name="new_password" id="new_password" required
                       minlength="5" autocomplete="new-password"
                       class="w-full bg-stone-800/60 backdrop-blur border border-stone-600/50 text-white placeholder-stone-400/50 px-4 py-3 rounded-xl outline-none focus:border-lime-400 focus:ring-2 focus:ring-lime-400/30 transition">
                <p class="text-stone-400/70 text-xs mt-1">At least 5 characters</p>
            </div>

            <div>
                <label for="new_password_confirmation" class="block text-xs font-semibold text-stone-300 mb-2">
