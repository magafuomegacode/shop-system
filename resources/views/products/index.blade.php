...
@section('content')
        ...
        <div class="relative z-20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-2 flex-wrap w-full sm:w-auto">
                <button type="button" id="download-trigger"
                        class="flex-1 sm:flex-initial flex-shrink-0 bg-blue-900/40 backdrop-blur border border-blue-700/50 hover:bg-blue-800/50 text-white text-sm font-semibold px-4 py-2.5 rounded-xl flex items-center justify-center gap-2 transition">
                ...
                <a href="{{ route('products.create') }}"
                   class="flex-1 sm:flex-initial flex-shrink-0 bg-gradient-to-br from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-white text-sm font-semibold px-4 py-2.5 rounded-xl flex items-center justify-center gap-2 transition shadow-lg">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
        ...
        <form method="GET" class="flex flex-col sm:flex-row gap-3">
            <div class="flex-1 relative">
                ...
                <input type="text" name="search" value="{{ request('search') }}"
                       ...
                       class="w-full pl-10 pr-4 py-2.5 rounded-xl outline-none text-sm bg-blue-900/40 backdrop-blur border border-blue-700/50 text-white placeholder-blue-200/50 focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/30 transition">
            </div>

            <select name="store" class="px-4 py-2.5 rounded-xl outline-none text-sm appearance-none min-w-[150px] bg-blue-900/40 backdrop-blur border border-blue-700/50 text-white focus:border-cyan-400">
            ...
            @if(request()->hasAny(['search', 'store']))
                <a href="{{ route('products.index') }}"
                   class="bg-blue-900/40 backdrop-blur hover:bg-blue-800/50 text-white text-sm font-semibold px-5 py-2.5 rounded-xl transition text-center border border-blue-700/50">
                    Clear
                </a>
            @endif
        </form>
                ...
                <table class="w-full border-collapse">
                    <thead>
                        <tr class="bg-blue-950 border-b-2 border-cyan-500/60">
                            ...
                            <th class="px-4 py-3 text-left text-xs font-bold text-white uppercase tracking-wide border-r border-blue-700/50">SKU</th>
                            <th class="px-4 py-3 text-left text-xs font-bold text-white uppercase tracking-wide border-r border-blue-700/50">Store</th>
                            <th class="px-4 py-3 text-left text-xs font-bold text-white uppercase tracking-wide border-r border-blue-700/50">Unit</th>
                            <th class="px-4 py-3 text-left text-xs font-bold text-white uppercase tracking-wide border-r border-blue-700/50">Size</th>
                    ...
                    <tbody>
                        @foreach($products as $i => $product)
                            ...
                            <tr class="border-b border-blue-700/30 hover:bg-blue-800/40 transition {{ $i % 2 === 0 ? 'bg-blue-900/40' : 'bg-blue-800/20' }}">
                                {{-- # --}}
                                <td class="px-4 py-3 text-sm text-white border-r border-blue-700/30">
                                ...
                                {{-- SKU --}}
                                <td class="px-4 py-3 text-sm text-white font-mono border-r border-blue-700/30">
                                    {{ $product->sku ?? '—' }}
                                </td>
                                ...
                                <td class="px-4 py-3 border-r border-blue-700/30">
                                    @if($product->store)
                                        <span class="px-2 py-0.5 bg-cyan-500/20 text-cyan-200 rounded-md text-xs font-medium border border-cyan-500/40">
                                            {{ $product->store->name }}
                                        </span>
                                    @else
                                        <span class="text-blue-200/60 text-xs">—</span>
                                    @endif
                                </td>
                                ...
                                {{-- Size --}}
                                <td class="px-4 py-3 text-sm text-white font-semibold border-r border-blue-700/30">
                                    {{ $sizeDisplay }}
                                </td>

                                {{-- Price --}}
                                <td class="px-4 py-3 text-sm text-white font-semibold text-right border-r border-blue-700/30">
                                    ...
                                    <div class="flex items-center justify-center gap-1">
                                        ...
                                        @if(auth()->user()->isAdmin() || auth()->user()->isOwner() || auth()->user()->isCashier())
                                            <a href="{{ route('products.edit', $product) }}"
                                               class="w-7 h-7 rounded-lg bg-blue-900/40 hover:bg-blue-800/50 border border-blue-700/50 flex items-center justify-center transition"
                                               title="Edit">
                                                <svg class="w-3.5 h-3.5 text-cyan-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
    ...
    @else
        <div class="bg-gradient-to-br from-blue-950 via-blue-900 to-blue-950 rounded-2xl p-10 text-center shadow-2xl border border-blue-700/50">
            ...
            <p class="text-blue-100 text-sm font-medium">No products yet</p>
            <a href="{{ route('products.create') }}" class="inline-block mt-4 text-cyan-400 hover:text-cyan-300 text-sm font-semibold">
        ...
        <div class="bg-gradient-to-br from-blue-950 via-blue-900 to-blue-950 rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden border border-blue-700/50">
            <div class="px-5 py-4 border-b border-blue-700/50 flex items-center justify-between bg-blue-950">
                <div>
                    ...
                    <p class="text-blue-200 text-xs mt-0.5">Choose a format</p>
                </div>
                <button type="button"
                ...
                <a href="{{ route('products.download.report', request()->only(['search', 'store'])) }}"
                   ...
                   class="flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-blue-800/40 transition">
                    ...
                    <div class="text-left flex-1">
                        ...
                        <p class="text-blue-200 text-xs">Printable · Grouped by store</p>
                    </div>
                ...
                <a href="{{ route('products.download.csv', request()->only(['search', 'store'])) }}"
                   ...
                   class="flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-blue-800/40 transition">
                    ...
                    <div class="text-left flex-1">
                        ...
                        <p class="text-blue-200 text-xs">For spreadsheets</p>
                    </div>
                    <svg class="w-4 h-4 text-blue-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/>
                    </svg>
