...
@section('content')
    ...
    <div class="bg-gradient-to-br from-stone-800 via-stone-700 to-stone-800 rounded-2xl p-5 mb-6 shadow-2xl border border-stone-600/50 relative overflow-hidden">
        ...
        <div class="relative z-10 flex items-center gap-3">
            ...
            <div class="min-w-0">
                ...
                <p class="text-stone-300 text-xs sm:text-sm mt-0.5 truncate">
    ...
    {{-- Errors --}}
    @if($errors->any())
        <div class="bg-red-500/20 border border-red-500/40 text-red-200 px-4 py-3 rounded-xl mb-6 text-sm">
    ...
    <form method="POST" action="{{ route('products.update', $product) }}"
        ...
        <div>
            <div class="flex items-center justify-between mb-2">
                ...
                <a href="{{ route('stores.create') }}"
                   class="inline-flex items-center gap-1 text-lime-300 hover:text-lime-200 text-xs font-semibold">
            ...
            <p class="text-stone-400/70 text-xs mt-1">Product is stored in this store</p>
        </div>
        ...
        <div class="grid grid-cols-2 gap-3">
            ...
            <div>
                ...
                <select name="unit" id="unit" required
                        class="w-full bg-stone-800/60 backdrop-blur border border-stone-600/50 text-white px-4 py-3 rounded-xl outline-none appearance-none focus:border-lime-400 focus:ring-2 focus:ring-lime-400/30 transition">
                    <option value="" class="bg-stone-800">-- Select Unit --</option>

                    <option value="pcs"    class="bg-stone-800" {{ old('unit', $product->unit) == 'pcs' ? 'selected' : '' }}>pcs (pieces)</option>
                    <option value="kg"     class="bg-stone-800" {{ old('unit', $product->unit) == 'kg' ? 'selected' : '' }}>kg (kilogram)</option>
                    <option value="g"      class="bg-stone-800" {{ old('unit', $product->unit) == 'g' ? 'selected' : '' }}>g (gram)</option>
                    <option value="litre"  class="bg-stone-800" {{ old('unit', $product->unit) == 'litre' ? 'selected' : '' }}>litre</option>
                    <option value="ml"     class="bg-stone-800" {{ old('unit', $product->unit) == 'ml' ? 'selected' : '' }}>ml (millilitre)</option>
                    <option value="box"    class="bg-stone-800" {{ old('unit', $product->unit) == 'box' ? 'selected' : '' }}>box</option>
                    <option value="pack"   class="bg-stone-800" {{ old('unit', $product->unit) == 'pack' ? 'selected' : '' }}>pack</option>
                    <option value="carton" class="bg-stone-800" {{ old('unit', $product->unit) == 'carton' ? 'selected' : '' }}>carton</option>
                    <option value="bottle" class="bg-stone-800" {{ old('unit', $product->unit) == 'bottle' ? 'selected' : '' }}>bottle</option>
                    <option value="can"    class="bg-stone-800" {{ old('unit', $product->unit) == 'can' ? 'selected' : '' }}>can</option>
                    <option value="sachet" class="bg-stone-800" {{ old('unit', $product->unit) == 'sachet' ? 'selected' : '' }}>sachet</option>
                    <option value="dozen"  class="bg-stone-800" {{ old('unit', $product->unit) == 'dozen' ? 'selected' : '' }}>dozen</option>
                    <option value="bundle" class="bg-stone-800" {{ old('unit', $product->unit) == 'bundle' ? 'selected' : '' }}>bundle</option>
                    <option value="bag"    class="bg-stone-800" {{ old('unit', $product->unit) == 'bag' ? 'selected' : '' }}>bag</option>
                    <option value="roll"   class="bg-stone-800" {{ old('unit', $product->unit) == 'roll' ? 'selected' : '' }}>roll</option>
                    <option value="metre"  class="bg-stone-800" {{ old('unit', $product->unit) == 'metre' ? 'selected' : '' }}>metre</option>
                    <option value="pair"   class="bg-stone-800" {{ old('unit', $product->unit) == 'pair' ? 'selected' : '' }}>pair</option>
                    <option value="set"    class="bg-stone-800" {{ old('unit', $product->unit) == 'set' ? 'selected' : '' }}>set</option>
                    <option value="other"  class="bg-stone-800" {{ old('unit', $product->unit) == 'other' ? 'selected' : '' }}>Other</option>
                </select>
            </div>
        </div>
        ...
        <div id="profit-preview" class="bg-green-500/20 border border-green-500/40 rounded-xl px-4 py-3 text-sm">
            <div class="flex items-center justify-between">
                <span class="text-stone-200 font-medium">Expected profit per unit:</span>
                <span id="profit-value" class="font-bold text-green-400">TSh 0</span>
            </div>
            <p class="text-xs text-stone-400/70 mt-1">Calculated from Selling Price − Cost Price</p>
        </div>
        ...
        <div class="flex items-center gap-2">
            ...
            <label for="is_active" class="text-sm text-stone-100">Active product</label>
        </div>
        ...
        <div class="flex items-center gap-3 pt-3">
            ...
            <a href="{{ route('products.index') }}"
               class="px-5 py-3 rounded-xl bg-stone-800/60 hover:bg-stone-700/60 border border-stone-600/50 text-stone-100 text-sm font-medium transition">
                Cancel
            </a>
        </div>
